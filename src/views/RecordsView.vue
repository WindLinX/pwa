...
const dayGroupElements = ref<Record<string, HTMLElement>>({})
...
async function load() { records.value = period.value === 'all' ? await recordService.listAll() : await recordService.listBetween(range.value.start, range.value.end); categories.value = await categoryService.list(); budget.value = await budgetService.status(anchor.value); await nextTick(); if (view.value === 'report') drawChart() }
...
async function selectCell(cell: CalendarCell) {
  anchor.value = cell.date
...
function calendarAmountClass(cell: CalendarCell) {
  if (!cell.amount || !budget.value?.configured) return ''
  return cell.amount > budget.value.dailyAmount ? 'over-budget' : 'within-budget'
}
